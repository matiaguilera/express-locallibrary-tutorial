...
exports.book_delete_post = function(req, res, next) {
    ...
    async.parallel({
        ...
        books_instances: function(callback) {
          Book.find({ 'book': req.body.bookid }).populate('book').exec(callback)
        },
    }, function(err, results) {
        if (err) { return next(err); }
        // Success
        if (results.books_instances.length > 0) {
            ...
            res.render('book_delete', { title: 'Delete Book', book: results.book, book_intances: results.books_instances } );
            return;
        }
        else {
